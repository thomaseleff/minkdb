...
def test_database_path_includes_mink_folder(temp_library):
    """User expects database in .minkdb subfolder."""
    db_path = get_database_path(temp_library)
    assert db_path.parent.name == ".minkdb"
    assert db_path.name == "catalog.json"
...
def test_catalog_output_excludes_unmatched(
    cli_runner,
    temp_library,
    itunes_xml_with_track,
):
    """User expects unmatched albums to not appear in output."""
    catalog_file = temp_library / ".minkdb" / "catalog.json"
